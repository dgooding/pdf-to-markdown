...
run=Path('artifacts/endpoint_release_check_live')/('run_'+str(int(time.time())))
...
parts.append((f'--{boundary}--\r\n').encode())
body=b''.join(parts)
req=request.Request(base+'/api/convert', data=body, method='POST', headers={'Content-Type':f'multipart/form-data; boundary={boundary}'})
...
job=payload['job_id']
for _ in range(160):
    with request.urlopen(base+f'/api/status/{job}', timeout=30) as r:
...
if s.get('status')!='completed':
    ...
    print('BLOCKER',f'job status {s}')
    raise SystemExit(1)

zip_path=run/'endpoint_package.zip'
with request.urlopen(base+f'/api/download/{job}', timeout=180) as r:
...
if compare_assets:
    for n in eas:
        h1=hashlib.sha256((endpoint_assets/n).read_bytes()).hexdigest()
        h2=hashlib.sha256((backend_assets/n).read_bytes()).hexdigest()
        if h1!=h2:
        ...
        'pages': [
            {
                ...
                'selected_candidate': pg.get('selected_candidate'),
                'technical_status': pg.get('technical_status'),
                'fidelity_status': pg.get('fidelity_status')
            }
...
em=json.loads(manifest_path.read_text(encoding='utf-8'))
bm=json.loads(backend_manifest.read_text(encoding='utf-8'))
compare_manifest = normalize_manifest(em)==normalize_manifest(bm)

eq=json.loads(quality_path.read_text(encoding='utf-8'))
bq=json.loads(backend_quality.read_text(encoding='utf-8'))
