# Published Documents

Documents published by the GitHub Actions converter appear here.

Repository administrators can use **Converter** to upload and publish documents.

## Available Documents

- [github-pages-smoke](github-pages-smoke/index.md) — `github-pages-smoke` <button type="button" class="delete-published-document" data-site-path="github-pages-smoke">Delete</button>
- [Service Desk Publish Smoke](user-manuals/service-desk-publish-smoke/index.md) — `user-manuals/service-desk-publish-smoke` <button type="button" class="delete-published-document" data-site-path="user-manuals/service-desk-publish-smoke">Delete</button>
